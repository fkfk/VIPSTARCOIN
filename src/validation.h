...
static const CAmount DEFAULT_GAS_PRICE=40;
static const CAmount MAX_RPC_GAS_PRICE=100;
 ...
 */
bool ProcessNewBlockHeaders(const std::vector<CBlockHeader>& block, CValidationState& state, const CChainParams& chainparams, const CBlockIndex** ppindex=NULL, const CBlockIndex** pindexFirst=NULL);
...
struct CAddressUnspentValue {
    ...
    CAddressUnspentValue(CAmount sats, CScript scriptPubKey, int height, bool isCoinBase, bool isStake) {
        satoshis = sats;
        script = scriptPubKey;
        blockHeight = height;
        coinBase = isCoinBase;
