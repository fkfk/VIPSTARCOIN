...
std::string LicenseInfo()
{
    ...
    return CopyrightHolders("Copyright (C) %i-%i") + "\n" +
           ...
           strprintf(_("Distributed under the GPLv3 license, see the %s"), "<https://www.gnu.org/licenses/gpl-3.0.html>") + "\n" +
           "\n" +
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                if (fReindex) {
                    boost::filesystem::path stateDir = GetDataDir() / "stateVIPSTARCOIN";
                ...
                boost::filesystem::path qtumStateDir = GetDataDir() / "stateVIPSTARCOIN";
