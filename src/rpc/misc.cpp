...
UniValue getdgpinfo(const JSONRPCRequest& request)
{
    ...
    obj.push_back(Pair("mingasprice", qtumDGP.getMinGasPrice(chainActive.Height())));
    obj.push_back(Pair("blockgaslimit", qtumDGP.getBlockGasLimit(chainActive.Height())));
...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total VIPSTARCOIN balance of the wallet\n"
            "  \"stake\": xxxxxxx,           (numeric) the total VIPSTARCOIN stake balance of the wallet\n"
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "\nReturn information about the given VIPSTARCOIN address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The VIPSTARCOIN address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\", (string) The VIPSTARCOIN address validated\n"
            ...
            "}\n"
            "\nExamples:\n"
            + HelpExampleCli("validateaddress", "\"VD5tffByCTgKtQssUxuSuykcsTV6PFULfE\"")
            + HelpExampleRpc("validateaddress", "\"VD5tffByCTgKtQssUxuSuykcsTV6PFULfE\"")
        );
...
UniValue getaddressutxos(const JSONRPCRequest& request)
{
    ...
    for (std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> >::const_iterator it=unspentOutputs.begin(); it!=unspentOutputs.end(); it++) {
        ...
        output.push_back(Pair("isCoinBase", it->second.coinBase));
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are VIPSTARCOIN addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) VIPSTARCOIN address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
            ...
            + HelpExampleCli("createmultisig", "2 \"[\\\"VD5tffByCTgKtQssUxuSuykcsTV6PFULfE\\\",\\\"VYpgSAGYsSwmFukkWzugzqr3BragHnJRdA\\\"]\"") +
            "\nAs a json rpc call\n"
            + HelpExampleRpc("createmultisig", "2, \"[\\\"VD5tffByCTgKtQssUxuSuykcsTV6PFULfE\\\",\\\"VYpgSAGYsSwmFukkWzugzqr3BragHnJRdA\\\"]\"")
        ;
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The VIPSTARCOIN address to use for the signature.\n"
            ...
            "\nAs json rpc\n"
            + HelpExampleRpc("verifymessage", "\"VD5tffByCTgKtQssUxuSuykcsTV6PFULfE\", \"signature\", \"my message\"")
        );
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw runtime_error(
            ...
            "VIPSTARCOIN-cli and the GUI. There is no server-side difference."
        );
